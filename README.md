# Interative UMD Course Plannning Tool

📍 Interactive visualization tool to help UMD students plan class schedules and commutes effecively. 

🔗 **Live Demo:** [zijie-cai.github.io/umd_schedule](https://zijie-cai.github.io/umd_schedule)  
🎥 **Demo Video:** [Watch Demo](https://www.youtube.com/watch?v=5BRXPktydIY)

## Features

- 🚌 Bus route overlay with class buildings
- 📚 Prerequisite tree visualization
- 👨‍🏫 Professor comparison (GPA, reviews, sentiment)
- 🗺️ Heatmap of class density across campus
- 📈 Trends in course GPA & enrollment over time

## Tech Stack

- D3.js, HTML, CSS, JavaScript
- UMD.io & PlanetTerp APIs
- GitHub Pages for deployment

## Team

- Zijie Cai  
- Tianfu Wang
- Haoming Cai  
